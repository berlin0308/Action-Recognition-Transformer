...
def get_labels(csv_dir_path):
    labels = []
    for file_path in csv_dir_path.iterdir():
        labels.append(file_path.name.split('.')[0])
    return sorted(list(set(labels)))


def convert_greenhouse_csv_to_json(csv_dir_path, video_dir_path,
                               dst_json_path):
...
if __name__ == '__main__':
    ...
    # dst_json_path = args.dst_dir_path / args.name
    dst_json_path = Path("calf_data/V8_10_0_txt/calf_annotation.json")
    convert_greenhouse_csv_to_json(Path("calf_data/V8_10_0_txt"), Path("calf_data/Dataset_V8"),
                                   dst_json_path)
