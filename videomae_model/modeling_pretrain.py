...
from videomae_model.modeling_finetune import (
    Block,
    PatchEmbed,
    _cfg,
...
@register_model
def pretrain_videomae_small_patch16_224(pretrained=False, pretrain_path=None,**kwargs):
...
@register_model
def pretrain_videomae_giant_patch14_224(pretrained=False, pretrain_path=None, **kwargs):
    model = PretrainVisionTransformer(
        img_size=224,
        patch_size=14,
...
if __name__ == '__main__':
    # model = pretrain_videomae_small_patch16_224(pretrained=True, pretrain_path="vit_s_k710_dl_from_giant.pth")
    model = pretrain_videomae_giant_patch14_224(pretrained=True, pretrain_path="vit_g_hybrid_pt_1200e.pth").to(device)
    dummy_input = torch.randn(1, 3, 16, 224, 224).to(device)
    model.eval()

    mask_percent = 0.25 # invisible for encoder

    input_shape_after_patch = (1, 2048)

    num_masked = int(torch.prod(torch.tensor(input_shape_after_patch)) * mask_percent)
    mask = torch.ones(input_shape_after_patch, dtype=torch.bool)

    mask.view(-1)[torch.randperm(mask.numel())[:num_masked]] = False
    print(mask.shape)

    # mask = torch.ones_like(torch.randn(1, 2048, 1408), dtype=torch.bool)
